...
def process_single_chunk(chunk, generator, validator):
    ...
    if not generated_quest_list:
        log_info("      -> Generator returned no results.")
        return []
    
    time.sleep(TIME_DELAY_SECONDS)

    log_info(f"      -> Generated {len(generated_quest_list)} Questions. Validating...")
    ...
    if not is_valid_list or len(is_valid_list) != len(generated_quest_list):
        log_info("      -> Validator returned an error or mismatched format.")
        return []
...
def create_quest_from_file(file_path):
    ...
    try:
        ...
        if os.path.exists(output_file_path):
            log_info("  -> Found existing output file. Loading checkpoint...")
            try:
                ...
                log_info(f"  -> Loaded {len(processed_chunk_ids)} processed chunks and {len(existing_questions)} existing questions.")
            except json.JSONDecodeError:
                log_warning(f"  -> Warning: Output file {output_file_path} is corrupted. Restarting.")
                validated_quests_for_this_file = []
                processed_chunk_ids = set()
                existing_questions = set()
        ...
        if not chunks_to_process:
            log_info(f"  -> All chunks already processed. Skipping file.")
            return (output_file_path, 0)
        
        log_info(f"  -> {len(chunks_to_process)}/{len(cleaned_chunks)} chunks remaining to process.")

        for i, chunk in enumerate(chunks_to_process):
            chunk_order_id = chunk.get('source', {}).get('chunk_order', f"index_{i}")
            log_info(f"    -> Processing chunk {chunk_order_id} ({i+1}/{len(chunks_to_process)})...")
            ...
            if new_valid_quests:
                ...
                if unique_new_quests:
                    ...
                    log_info(f"      -> Accept {len(unique_new_quests)} valid, unique Questions. Saving checkpoint...")
                    ...
                    log_info(f"      -> Valid Questions found but all were duplicates.")
            else:
                log_info(f"    -> No valid Questions found for this chunk.")

    except KeyboardInterrupt:
        return (output_file_path, quests_created_count)
    except Exception as e:
        log_warning(f"Processing file {file_name}: {e}")
...
def create_quest_from_folder(input_dir=CLEANED_DIR, output_dir=GENERATED_DIR):
    if not os.path.exists(input_dir):
        log_error(f"Cleaned data directory not found: {input_dir}")
        return
...
if __name__ == "__main__":
    try:
        log_info("========== Loading LLM Configuration ==========")        
        log_info("\n========== Starting Generation Pipeline ==========")
        create_quest_from_folder(CLEANED_DIR, GENERATED_DIR)
        log_info("\n========== Generation Pipeline Finished ==========")
        
    except Exception as e:
        log_error(f"Failed to load LLM instances or run pipeline: {e}")