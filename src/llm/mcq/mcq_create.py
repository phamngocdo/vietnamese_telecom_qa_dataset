...
GENERATED_DIR = os.path.join(PROJECT_ROOT, "data/generated/mcq/add")
...
def load_model(model_config, prompt_lang, is_validator=False, api_key=None):
    llm_class = model_config.get("class").lower()
    model = model_config.get("model")
    if "openai" in llm_class:
        return OpenAILLM(model=model, key=api_key, prompt_lang=prompt_lang, is_validator=is_validator)
    elif "gemini" in llm_class:
        return GeminiLLM(model=model, key=api_key, prompt_lang=prompt_lang, is_validator=is_validator)
    elif "huggingface" in llm_class:
        return HuggingFaceLLM(model=model, prompt_lang=prompt_lang, is_validator=is_validator)
    else:
...
def process_single_chunk(chunk, generator, validator, time_delay_seconds=10):
    ...
    print(f"      -> Generated {len(generated_mcq_list)} MCQs. Validating...")

    time.sleep(time_delay_seconds)
    
    is_valid_list = validator.validate_mcq(generated_mcq_list)
    if not is_valid_list or len(is_valid_list) != len(generated_mcq_list):
        print("      -> Validator returned an error or mismatched format.")
        return []

    validated_mcqs = []

    for i, mcq in enumerate(generated_mcq_list):
...
if __name__ == "__main__":
    def create_mcq_from_folder(generator, validator, input_dir=CLEANED_JSON_DIR, output_dir=GENERATED_DIR, time_delay_seconds=5):
        ...
        for root, _, files in os.walk(input_dir):
            ...
            for file_name in files:
                ...
                try:
                    ...
                    validated_mcqs_for_this_file = []
                    processed_chunk_ids = set()
                    if os.path.exists(output_file_path):
                        print("  -> Found existing output file. Loading checkpoint...")
                        try:
                            with open(output_file_path, "r", encoding="utf-8") as f:
                                validated_mcqs_for_this_file = json.load(f)
                                processed_chunk_ids = {mcq['source']['chunk_order'] for mcq in validated_mcqs_for_this_file if 'chunk_order' in mcq['source']}
                    ...
                    for i, chunk in enumerate(chunks_to_process):
                        ...
                        new_valid_mcqs = process_single_chunk(chunk, generator, validator, time_delay_seconds)
                        
                        if new_valid_mcqs:
                            count_mcq_for_file += len(new_valid_mcqs)
                            print(f"      -> Accept {len(new_valid_mcqs)} valid MCQs. Saving checkpoint...")
                            validated_mcqs_for_this_file.extend(new_valid_mcqs)
                            
                            with open(output_file_path, "w", encoding="utf-8") as f:
                                json.dump(validated_mcqs_for_this_file, f, ensure_ascii=False, indent=4)
                        else:
                            print(f"    -> No valid MCQs found for this chunk.")
                    ...
                    total_mcq += count_mcq_for_file
                    time_end = (time.time() - time_start) / 60
                    print(f"Progress saved {count_mcq_for_file} MCQs to {output_file_path} in {time_end:.2f} minutes.")

        total_time = (time.time() - time_start) / 60
        print(f"\nProgress completed. Progress created {total_mcq} MCQs in {total_time:.2f} minutes.")

    try:        
        print("\n========== Starting MCQ Generation Pipeline ==========")
        create_mcq_from_folder(generator_llm, validator_llm, CLEANED_JSON_DIR, GENERATED_DIR)
        
        print("\n========== MCQ Generation Pipeline Finished ==========")
